    ...
    internal class MySqlQuery : BaseQuery
    {
        ...
        protected override void InsertCommandText(bool IsOutIdentity = false)
        {
            ...
            foreach (FiledProperty filed in bagPar.PropertyFileds)
            {
                ...
                if ((filed.isKey && !filed.isIdentity) || (!filed.isIdentity))
                {
                    ...
                    this.Parameters.Add("@" + filed.filedName + shortguid, ((val == null && filed.isNull) || (filed.typeName.ToLower() == "string" && string.IsNullOrWhiteSpace(val == null ? string.Empty : val.ToString()))) ? DBNull.Value : val, CoreUtils.SqlTypeString2SqlType(filed.typeName.ToLower()));
                }
            }
        ...
        protected override void DeleteCommandText()
        {
            ...
            else if (bagPar.data != null)
            {
                ...
                var shortguid = "_" + Bitter.Tools.Utils.GuidExtends.ShortGuid();
                this.Parameters.Add("@" + bagPar.keyName + shortguid, ((keyFiled == null && keyFiled.isNull) || (keyFiled.typeName.ToLower() == "string" && string.IsNullOrWhiteSpace(keyFiled.value == null ? string.Empty : keyFiled.value.ToString()))) ? DBNull.Value : keyFiled.value, CoreUtils.SqlTypeString2SqlType(keyFiled.typeName.ToLower()));
