        ...
        private static Int32 GetSubmit(BaseQuery o, string targetdb,out Exception outex)
        {
            ...
            try
            {
                if (o.excutParBag.excutEnum == ExcutEnum.Insert && ((ExcutParBag_Insert)o.excutParBag).isOutIdentity)
                {
                    ...
                    var aff = DataAccess.ExecuteScalarToWriterOnlyForInsertRentrunIdentity(o, o.databaseProperty);

                    if (!((ExcutParBag_Insert)o.excutParBag).isIdentityModel)
                    {
                        return 1;
                    }
                    return affectedCount=aff.ToSafeInt32(-1);
                }
                else
                {
